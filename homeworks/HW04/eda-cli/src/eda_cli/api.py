...
# ---------- Собственный эндпоинт: полный набор флагов качества ----------

@app.post(
    "/quality-flags-from-csv",
    response_model=QualityResponse,
    tags=["quality"],
    summary="Возвращает полный набор флагов качества из CSV с HW03 эвристиками",
)
async def quality_flags_from_csv(file: UploadFile = File(...)) -> QualityResponse:
    ...
    summary = summarize_dataset(df)
    missing_df = missing_table(df)
    # Исправлено: теперь передаём df как третий аргумент
    flags_all = compute_quality_flags(summary, missing_df, df)

    # Для наглядности оставим качество как float (если есть), иначе 0
    score = float(flags_all.get("quality_score", 0.0))
    score = max(0.0, min(1.0, score))
    ok_for_model = score >= 0.7

    # Флаги должны включать все эвристики из HW03
    flags_bool: dict[str, bool] = {
        key: bool(value)
        for key, value in flags_all.items()
        if isinstance(value, bool)
    ...
    print(
        ...
        f"n_rows={n_rows} n_cols={n_cols} latency_ms={latency_ms:.1f} ms"
    )

    return QualityResponse(
        ok_for_model=ok_for_model,
        quality_score=score,
        message="Возвращен полный набор флагов качества (по эвристикам HW03).",
        latency_ms=latency_ms,
        flags=flags_bool,
        dataset_shape={"n_rows": n_rows, "n_cols": n_cols},
    )
