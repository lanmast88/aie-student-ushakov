...
- **Название проекта:** `Сервис рекомендаций книг на основе пользовательских оценок`
- **Автор:** `Ушаков Глеб Максимович`
- **Группа:** `ЭФБО-06-24`
- **Контакт:** `@lanmast8`

- **Краткое описание (2-4 предложения):**  
  `Проект посвящён построению сервиса персонализированных рекомендаций книг на основе пользовательских оценок.
Используются открытые датасеты, методы коллаборативной фильтрации и baseline-модели популярности, а также offline-оценка качества рекомендаций.
Результат — REST API, которое по идентификатору пользователя возвращает Top-K рекомендованных книг.`
...
Для обучения используется открытый датасет Goodbooks-10k.
...
- Запущу сервис через python -m src.service и продемонстрирую работу REST API с получением Top-K рекомендаций книг для конкретного пользователя.

- Покажу ноутбук с основными экспериментами, сравнением моделей по метрикам Precision@K и Recall@K, а также короткий анализ результатов.
