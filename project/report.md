...
- **Группа:** `ЭФБО-06-24`
- **Контакт:** `@lanmast8`
- **Ссылка на репозиторий:** `[<URL GitHub-репозитория со студенческим шаблоном>](https://github.com/lanmast88/aie-student-ushakov.git)`
...
1. **Предметную область и задачу:**
   Сервис предназначен для рекомендации книг пользователям на основе их истории оценок и предпочтений.
   Тип задачи: рекомендация / ранжирование (recommender system / ranking).
   Целевая аудитория: пользователи, которые читают книги, оценивают их и хотят получать персонализированные рекомендации.
   Сценарий использования: пользователь открывает платформу, сервис анализирует его историю взаимодействий и формирует Top-K персонализированных рекомендаций, позволяя быстро находить интересные книги и экономить время на поиске контента.
